...
droplet_transport<-function(){
  ...
  for (i in 1:23) {
    ...
    if(i>=11){ddd<-1.02} # Why is ddd Changing
    if(i>=13){ddd<-1.05} # Why is ddd Changing
    if(i>=17){ddd<-1.04} # Why is ddd Changing
    ...
    # Solve the system
    out   <- ode(yini, times, EqnSys)  #AV rerified results for Z, X are correct;  AV 11202019
    #  summary(out)
    #  plot(out)
  ...
  data.frame(Dp[1:23],Xdist)  # AV checked result and is correct
  # Stop the clock
  proc.time() - ptm
