...
RUN git clone --branch v3.8 https://bitbucket.org/blaze-lib/blaze.git /tmp/blaze \
	&& cmake -S /tmp/blaze -B /tmp/blaze-build -G Ninja -DCMAKE_BUILD_TYPE=Release -DUSE_LAPACK=Off \
...
RUN git clone --branch 3.4.0 https://gitlab.com/libeigen/eigen.git /tmp/eigen \
	&& cmake -S /tmp/eigen -B /tmp/eigen-build -G Ninja -DCMAKE_BUILD_TYPE=Release \
