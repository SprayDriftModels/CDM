...
std::optional<double> ZAtDistanceSingleNozzle(const DropletTransport::TrajectoryXZ& trajectory, double targetDistance)
{
    if (trajectory.empty()) {
        return std::nullopt;
    }
...
bool IntersectsHeightCellSingleNozzle(double z0, double z1, double lowerHeight, double upperHeight)
{
    ...
    const std::array<bool, constants::ns>& sflags)
{
    ...
    const auto classVolumes = ComputeClassVolumesFromCdfBins(volumeSprayed, 1, dp, dsd, dsdmodel);
    ...
    for (size_t distanceIdx = 0; distanceIdx < distances.size(); ++distanceIdx) {
        ...
        for (size_t streamline = 0; streamline < constants::ns; ++streamline) {
            ...
            for (size_t droplet = 0; droplet < count; ++droplet) {
                const auto z0 = ZAtDistanceSingleNozzle(trajectories[droplet], previousDistance);
                const auto z1 = ZAtDistanceSingleNozzle(trajectories[droplet], currentDistance);
                ...
                for (size_t heightIdx = 0; heightIdx < heights.size(); ++heightIdx) {
                    ...
                    if (!IntersectsHeightCellSingleNozzle(*z0, *z1, lowerHeight, upperHeight)) {
                        continue;
                    }
